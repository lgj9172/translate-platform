...
import type { CreateBlogCommentDto, CreateBlogDto, UpdateBlogDto } from "./blogs.dto";
...
export class BlogsService {
  ...
  async findAll(query: { start?: number; size?: number }) {
    ...
    const [data, total_count] = await this.prisma.$transaction([
      this.prisma.blog.findMany({
        ...
        skip,
        take,
        include: { user: { select: { name: true, avatar: true } }, _count: { select: { comments: true } } },
  ...
  async update(blogId: string, userId: string, dto: UpdateBlogDto) {
    const blog = await this.prisma.blog.findUnique({ where: { blog_id: blogId } });
    if (!blog) throw new NotFoundException();
    if (blog.user_id !== userId) throw new ForbiddenException();
    const updated = await this.prisma.blog.update({ where: { blog_id: blogId }, data: dto });
    return ok(updated);
  }
  ...
  async remove(blogId: string, userId: string) {
    ...
    if (!blog) throw new NotFoundException();
    if (blog.user_id !== userId) throw new ForbiddenException();
    await this.prisma.blog.update({ where: { blog_id: blogId }, data: { is_deleted: true } });
    return ok(null);
  }
  ...
  async removeComment(commentId: string, userId: string) {
    ...
    await this.prisma.blogComment.delete({ where: { comment_id: commentId } });
    return ok(null);
  }
}
